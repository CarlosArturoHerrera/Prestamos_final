...
export function SegmentsTabs({
  ...
  const loadClientsData = async () => {
    ...
    try {
      const res = await fetch("/api/dashboard/clients-segment", {
        cache: "no-store",
      });
  ...
  const loadLoansData = async () => {
    ...
    try {
      const res = await fetch("/api/dashboard/loans-segment", {
        cache: "no-store",
      });
  ...
  const loadNotificationsData = async () => {
    ...
    try {
      const res = await fetch("/api/dashboard/notifications-segment", {
        cache: "no-store",
      });
  ...
  return (
    <section className="space-y-4 animate-in fade-in slide-in-from-bottom-6 duration-700">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <p className="text-muted-foreground text-sm">
            Profundiza por segmento
          </p>
          <h2 className="text-xl font-semibold">
            Clientes, préstamos y alertas
          </h2>
        </div>
      ...
      <Tabs defaultValue="clients" className="space-y-4">
        <TabsList>
          <TabsTrigger value="clients" onClick={loadClientsData}>
            Clientes
          </TabsTrigger>
          <TabsTrigger value="loans" onClick={loadLoansData}>
            Préstamos
          </TabsTrigger>
          <TabsTrigger value="notifications" onClick={loadNotificationsData}>
            Notificaciones
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="clients" className="space-y-4">
          <Card className="bg-card/80 backdrop-blur">
            <CardHeader>
              ...
              <CardDescription>
                Estado de la cartera prioritaria por segmento.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loadingClients ? (
        ...
        <TabsContent value="loans">
          <Card className="bg-card/80 backdrop-blur">
            <CardHeader>
              ...
              <CardDescription>
                Próximos desembolsos y estatus operativo.
              </CardDescription>
            </CardHeader>
                ...
                <>
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                    ...
                    <Badge variant="outline">
                      Tiempo medio de aprobación 36h
                    </Badge>
                  </div>
                </>
              )}
          ...
          {loadingNotifications ? (
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {notifications.map((item, idx) => (
                <Card
                  key={`${item.title}-${idx}`}
                  className="bg-card/80 border-dashed backdrop-blur"
                >
