...
} from "@/components/dashboard/loan-edit-dialog";
...
export function CarteraTab() {
    ...
    const load = async () => {
      try {
        ...
        const res = await fetch(
          `/api/loans${params.toString() ? `?${params.toString()}` : ""}`,
          {
            cache: "no-store",
          },
        );
        ...
        const data: LoanRow[] = (Array.isArray(raw) ? raw : []).map(
          (row: any) => ({
            id: row.id,
  ...
  const stats = useMemo(() => {
    ...
    const active = loans.filter((l) =>
      l.status.toLowerCase().includes("activo"),
    ).length;
    const inReview = loans.filter(
      (l) => !l.status.toLowerCase().includes("activo"),
    ).length;
  ...
  const handleCreate = async (values: LoanFormValues) => {
    ...
    try {
      ...
      console.error(error);
      const message =
        error instanceof Error ? error.message : "Error al crear préstamo";
      toast.error(message);
    } finally {
      setIsCreating(false);
    }
  ...
  const handleUpdate = async (values: LoanEditFormValues) => {
    try {
      ...
      console.error(error);
      const message =
        error instanceof Error ? error.message : "Error al actualizar préstamo";
      toast.error(message);
    }
  };
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {loans.length
                  ? ((stats.active / loans.length) * 100).toFixed(0)
                  : 0}
                % de la cartera
              </p>
            </div>
            ...
            <LoanCreateDialog
              trigger={
                <Button
                  variant="default"
                  size="sm"
                  disabled={clients.length === 0 || isCreating}
                >
              ...
              <tbody>
                {isLoading ? (
                  <tr>
                    <td
                      colSpan={8}
                      className="h-14 px-4 text-center text-muted-foreground"
                    >
                      Cargando préstamos...
                    </td>
                  </tr>
                  ...
                  loans.map((loan) => (
                    ...
                    >
                      <td className="h-14 px-4 align-middle font-medium text-xs">
                        {loan.id}
                      </td>
                          ...
                          variant={
                            loan.status.toLowerCase().includes("activo")
                              ...
                              : loan.status
                                    .toLowerCase()
                                    .includes("revision") ||
                                  loan.status.toLowerCase().includes("revisión")
                                ? "secondary"
                                : "outline"
                          }
                        >
