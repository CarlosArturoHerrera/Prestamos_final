...
export const ReportsTab = forwardRef<ReportsTabRef>((_, ref) => {
  ...
  const stats = useMemo(() => {
    ...
    const avgRate =
      totalLoans > 0
        ? loans.reduce((sum, l) => sum + (Number(l.interest_rate) || 0), 0) /
          totalLoans
        : 0;

    const statusBreakdown = loans.reduce(
      (acc, loan) => {
    ...
    const clientLoans = loans.reduce(
      (acc, loan) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ref={reportRef}
        data-report-content
        className="space-y-6 rounded-[2rem] border border-border/70 bg-background p-8 text-foreground shadow-[0_20px_50px_rgba(15,23,42,0.08)]"
      >
        {/* Encabezado del reporte */}
        <div className="border-b border-border/80 pb-4">
          <h1 className="text-2xl font-bold text-foreground">
            Préstamos Elicar — Reporte de Préstamos
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {isMounted
              ? `Generado el ${formatSafeDate(new Date(), "dd 'de' MMMM 'de' yyyy")}`
              : "Generando reporte..."}
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="rounded-2xl border border-border/80 bg-surface p-4 shadow-sm">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Promedio: $
                {(stats.avgAmount || 0).toLocaleString("es-MX", {
          ...
          <div className="rounded-2xl border border-border/80 bg-surface p-6 shadow-sm">
            <div className="space-y-3">
              {Object.entries(stats.statusBreakdown).map(([status, count]) => {
                ...
                const bgColor =
                  statusColors[status.toLowerCase()] || "bg-slate-400";
        ...
        <div className="border-t border-border/80 pt-4 text-center text-xs text-muted-foreground">
          <p>
            Reporte generado automáticamente por Sistema de Gestión de Préstamos
          </p>
          <p className="mt-1">
            © {isMounted ? new Date().getFullYear() : "2026"} - Todos los
            derechos reservados
          </p>
        </div>
      </div>
