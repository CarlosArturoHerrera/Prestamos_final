...
export function ClientsTab() {
  ...
  const heroStats = useMemo(() => {
    ...
    const active = clients.filter((client) =>
      client.status.toLowerCase().includes("al día"),
    ).length;
    return {
      total,
      active,
  ...
  return (
    <div className="space-y-6">
      <ClientsHeroSection
        totalClients={heroStats.total}
        activeClients={heroStats.active}
      />
      ...
      <Card className="bg-card/80 dark:border-border/40 backdrop-blur animate-in fade-in slide-in-from-bottom-6 duration-700">
        <CardHeader>
          ...
          <CardDescription>
            Administra tu cartera de clientes con búsqueda, filtros y acciones
            rápidas
          </CardDescription>
        </CardHeader>
        <CardContent>
