...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "surface-panel bg-card text-card-foreground flex flex-col gap-6 rounded-2xl py-4 md:py-6",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex items-center px-4 md:px-6 [.border-t]:border-border [.border-t]:pt-6", className)}
      {...props}
    />
  )
}
