...
function badgeVariantResultado(
  codigo: string,
...
export function GestionCobranzaPanel({
  clienteId,
  prestamoId,
  prestamosOpciones,
  className,
  ...
  const [resultado, setResultado] =
    useState<ResultadoGestionCobranza>("CONTACTADO");
  const [prestamoIdForm, setPrestamoIdForm] = useState<string>("");

  const basePath =
    prestamoId != null
      ? `/api/prestamos/${prestamoId}/gestion-cobranza`
      : `/api/clientes/${clienteId}/gestion-cobranza`;
  ...
  const enviar = async () => {
    ...
    const body: Record<string, unknown> = {
      ...
      promesaMonto: promesaMonto.trim() ? promesaMonto : undefined,
  ...
  const showPrestamoPicker =
    prestamoId == null && (prestamosOpciones?.length ?? 0) > 0;

  const titulo = useMemo(
    () =>
      prestamoId != null
        ? "Seguimiento de cobranza (este préstamo)"
        : "Seguimiento de cobranza (cliente)",
    [prestamoId],
  );
  ...
  return (
    <Card className={cn("border-border/60 shadow-sm", className)}>
      <CardHeader>
        ...
        <CardDescription>
          Notas operativas, promesas de pago y próximo contacto. No modifica
          montos ni abonos del préstamo.
        </CardDescription>
      </CardHeader>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2 sm:col-span-2">
              ...
              <Select
                value={resultado}
                onValueChange={(v) =>
                  setResultado(v as ResultadoGestionCobranza)
                }
              >
            ...
            {showPrestamoPicker ? (
              <div className="space-y-2 sm:col-span-2">
                ...
                <Select
                  value={prestamoIdForm || "__none__"}
                  onValueChange={(v) =>
                    setPrestamoIdForm(v === "__none__" ? "" : v)
                  }
                >
                  <SelectTrigger className="w-full max-w-md">
            ...
            <div className="space-y-2">
              ...
              <CurrencyInput
                value={promesaMonto}
                onChange={(raw) => setPromesaMonto(raw)}
        ...
        <div>
          ...
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">
              Aún no hay registros de gestión.
            </p>
          ) : (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge
                          variant={badgeVariantResultado(row.resultado)}
                          className="text-xs"
                        >
                      ...
                      {row.prestamo_id != null ? (
                        <Button
                          ...
                          asChild
                        >
                          <Link href={`/prestamos/${row.prestamo_id}`}>
                            Préstamo #{row.prestamo_id}
                          </Link>
                        </Button>
                      ) : (
                    ...
                    {pr ? (
                      <p className="mt-1 text-xs text-muted-foreground">
                        Estado préstamo: {String(pr.estado ?? "—")} · Capital
                        pend.{" "}
                        {pr.capital_pendiente != null
                          ? formatRD(pr.capital_pendiente)
                          : "—"}
                      </p>
                    ) : null}
                    {row.notas ? (
                    ...
                    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                      {row.promesa_monto != null &&
                      Number(row.promesa_monto) > 0 ? (
                        <span>Promesa: {formatRD(row.promesa_monto)}</span>
                      ) : null}
                      {row.promesa_fecha ? (
                        <span>Fecha promesa: {row.promesa_fecha}</span>
                      ) : null}
