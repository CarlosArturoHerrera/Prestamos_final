...
const moneyString = z.union([
  z
    .string()
    .transform(unformatCurrency)
    .pipe(z.string().regex(/^\d+(\.\d{1,4})?$/, "Monto inválido")),
  z.number().transform((n) => String(n)),
]);
const positiveMoney = moneyString.refine(
  (v) => Number(v) > 0,
  "Debe ser un monto mayor que 0",
);

const phoneField = (msg = "Teléfono obligatorio") =>
  z.string().min(1, msg).max(50).transform(unformatPhone);
...
const cedulaField = (msg = "Cédula obligatoria") =>
  z.string().min(1, msg).max(50).transform(unformatCedula);
...
export const tipoPlazoSchema = z.enum([
  "DIARIO",
  "SEMANAL",
  "QUINCENAL",
  "MENSUAL",
...
const abonoCreateSchemaInner = z
  .object({
  ...
  .refine(
    (d) =>
      d.montoCapitalDebitado !== undefined || d.interesRecibido !== undefined,
    {
...
export const notificacionEnviarSchema = z
  ...
  .refine(
    (d) =>
      d.enviarATodos === true ||
      (d.representanteIds && d.representanteIds.length > 0),
    {
