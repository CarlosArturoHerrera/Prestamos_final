...
function drawPageHeader(doc: jsPDF, userName: string, now: Date, filters: { fechaDesde?: string; fechaHasta?: string }): number {
  const H = 38
  sf(doc, C.headerBg); doc.rect(0, 0, PAGE_W, H, "F")
  sf(doc, C.accent); doc.circle(ML + 9, H / 2, 8, "F")
  doc.setFont("helvetica", "bold"); doc.setFontSize(9); st(doc, C.white)
  doc.text("MP", ML + 9, H / 2 + 1.2, { align: "center" })
  doc.setFont("helvetica", "bold"); doc.setFontSize(17); st(doc, C.white)
  doc.text("Préstamos Elicar", ML + 22, H / 2 - 2.5)
  doc.setFont("helvetica", "normal"); doc.setFontSize(8.5); st(doc, C.headerSub)
  doc.text("Microfinanzas y Soluciones Crediticias", ML + 22, H / 2 + 5.5)
  const nowStr = now.toLocaleString("es-DO", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })
  doc.setFontSize(8); st(doc, C.headerSub)
  let ry = H / 2 - 7.5
  doc.text(`Generado: ${nowStr}`, PAGE_W - ML, ry, { align: "right" }); ry += 5.5
  doc.text(`Usuario: ${userName}`, PAGE_W - ML, ry, { align: "right" }); ry += 5.5
  if (filters.fechaDesde || filters.fechaHasta) {
    ...
    doc.text(`Período: ${desde} — ${hasta}`, PAGE_W - ML, ry, { align: "right" })
  }
  sf(doc, C.accent); doc.rect(0, H, PAGE_W, 3, "F")
  return H + 3
}
function drawMiniHeader(doc: jsPDF): number {
  sf(doc, C.headerBg); doc.rect(0, 0, PAGE_W, 16, "F")
  sf(doc, C.accent);   doc.rect(0, 16, PAGE_W, 2, "F")
  ...
  doc.text("Microfinanzas y Soluciones Crediticias — continuación", PAGE_W - ML, 11, { align: "right" })
  return 22
}
function drawKPIs(doc: jsPDF, startY: number, kpis: { totalPrestado: string; totalCobrado: string; totalPendiente: string; activos: number; mora: number; clientes: number }): number {
  let y = startY + 7
  doc.setFont("helvetica", "bold"); doc.setFontSize(10); st(doc, C.sectionBg)
  doc.text("RESUMEN EJECUTIVO", ML, y); y += 5
  const gap = 4; const cardW = (CW - gap * 2) / 3; const cardH = 23
  const items = [
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const cx = ML + col * (cardW + gap); const cy = y + row * (cardH + gap)
    sf(doc, C.kpiBg); doc.roundedRect(cx, cy, cardW, cardH, 2, 2, "F")
    sf(doc, items[i].color); doc.rect(cx, cy, 3.5, cardH, "F")
    sd(doc, C.kpiBorder); doc.setLineWidth(0.2); doc.roundedRect(cx, cy, cardW, cardH, 2, 2, "S")
    doc.setFont("helvetica", "bold"); doc.setFontSize(12); st(doc, items[i].color)
...
interface ColDef { header: string; key: string; w: number; align?: "left" | "right" | "center"; fmt?: (v: unknown) => string }
function drawSection(doc: jsPDF, startY: number, title: string, badge: string, cols: ColDef[], rows: Record<string, unknown>[], totalRow: Record<string, unknown> | null, opts: { warningMode?: boolean; addPage: () => number }): number {
  const { warningMode = false, addPage } = opts
  const ROW_H = 7
  let y = startY + 7
  if (y + 35 > FOOTER_Y) y = addPage()
  sf(doc, warningMode ? C.moraHead : C.sectionBg); doc.rect(ML, y - 5.5, CW, 10, "F")
  doc.setFont("helvetica", "bold"); doc.setFontSize(9.5); st(doc, C.white)
  doc.text(title, ML + 3, y + 0.5)
  doc.setFont("helvetica", "normal"); doc.setFontSize(7.5); st(doc, C.headerSub)
  doc.text(badge, PAGE_W - ML - 3, y + 0.5, { align: "right" }); y += 6
  sf(doc, warningMode ? [220, 38, 38] as RGB : C.tblHead); doc.rect(ML, y, CW, ROW_H, "F")
  ...
  for (const col of cols) {
    ...
    doc.text(col.header, tx, y + 4.8, { align: col.align === "right" ? "right" : col.align === "center" ? "center" : "left" }); cx += col.w
  }
  y += ROW_H
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const alt = i % 2 === 1
    sf(doc, warningMode ? (alt ? C.moraRowB : C.moraRowA) : (alt ? C.tblAlt : C.white)); doc.rect(ML, y, CW, ROW_H, "F")
    sd(doc, C.border); doc.setLineWidth(0.1); doc.line(ML, y + ROW_H, ML + CW, y + ROW_H)
    st(doc, C.body); cx = ML
    for (const col of cols) {
      const val = rows[i][col.key]; const text = col.fmt ? col.fmt(val) : (val == null ? "—" : String(val))
      const tx = col.align === "right" ? cx + col.w - 2 : col.align === "center" ? cx + col.w / 2 : cx + 2
      doc.text(text, tx, y + 4.8, { align: col.align === "right" ? "right" : col.align === "center" ? "center" : "left", maxWidth: col.w - 3 }); cx += col.w
    }; y += ROW_H
  }
  if (totalRow) {
    if (y + ROW_H + 2 > FOOTER_Y) y = addPage()
    sf(doc, C.totalBg); doc.rect(ML, y, CW, ROW_H + 1, "F")
    doc.setFont("helvetica", "bold"); doc.setFontSize(8); st(doc, C.totalTxt); cx = ML
    for (const col of cols) {
      const val = totalRow[col.key]
      if (val != null) { const text = col.fmt ? col.fmt(val) : String(val); const tx = col.align === "right" ? cx + col.w - 2 : col.align === "center" ? cx + col.w / 2 : cx + 2; doc.text(text, tx, y + 5.3, { align: col.align === "right" ? "right" : col.align === "center" ? "center" : "left" }) }; cx += col.w
    }; y += ROW_H + 1
  }
  return y
}
function stampFooters(doc: jsPDF, now: Date) {
  ...
  for (let p = 1; p <= total; p++) {
    doc.setPage(p); sd(doc, C.border); doc.setLineWidth(0.3); doc.line(ML, FOOTER_Y - 2, PAGE_W - ML, FOOTER_Y - 2)
...
// biome-ignore lint: any needed for exceljs dynamic shape
function xFont(opts: { bold?: boolean; size?: number; color?: string; italic?: boolean }): any {
  return { name: "Calibri", bold: opts.bold ?? false, size: opts.size ?? 11, italic: opts.italic ?? false, ...(opts.color ? { color: { argb: opts.color } } : {}) }
}
// biome-ignore lint: any needed for exceljs dynamic shape
function xBorder(c = XL.bdr): any {
  const s = { style: "thin", color: { argb: c } }; return { top: s, left: s, bottom: s, right: s }
}
// biome-ignore lint: any needed for exceljs dynamic shape
...
function xCell(sheet: any, ref: string, opts: { value?: unknown; fill?: string; font?: any; align?: any; border?: any; numFmt?: string }) {
...
function xMerge(sheet: any, range: string, opts: { value?: unknown; fill?: string; font?: any; align?: any; border?: any; numFmt?: string }) {
...
// biome-ignore lint: any needed for exceljs sheet
function xlHeader(sheet: any, numCols: number, reportTitle: string, userName: string, nowStr: string, filters: { fechaDesde?: string; fechaHasta?: string }): number {
  const L = colLetter(numCols)
  sheet.getRow(1).height = 32
  xMerge(sheet, `A1:${L}1`, { value: "Préstamos Elicar", fill: XL.hdrBg, font: xFont({ bold: true, size: 18, color: XL.hdrTxt }), align: xAlign("left", 2) })
  sheet.getRow(2).height = 18
  xMerge(sheet, `A2:${L}2`, { value: "Microfinanzas y Soluciones Crediticias", fill: XL.hdrBg, font: xFont({ size: 10, color: XL.hdrSub }), align: xAlign("left", 2) })
  sheet.getRow(3).height = 22
  xMerge(sheet, `A3:${L}3`, { value: reportTitle, fill: XL.accBg, font: xFont({ bold: true, size: 11, color: XL.accTxt }), align: xAlign("center") })
  ...
  xMerge(sheet, `A4:${L}4`, { value: info, fill: XL.lgray, font: xFont({ size: 9, color: XL.muted }), align: xAlign("left", 1) })
...
interface XlCol { header: string; key: string; width: number; align?: "left" | "right" | "center"; fmt?: string }
...
function xlTable(
  ...
  for (let i = 0; i < rows.length; i++) {
    const alt = i % 2 === 1
    const bg = warningMode ? (alt ? XL.moraB : XL.moraA) : (alt ? XL.tblAlt : XL.white)
    ...
    for (let j = 0; j < cols.length; j++) {
      ...
      cell.alignment = xAlign(col.align ?? "left", col.align !== "right" ? 1 : 0)
      cell.border = { bottom: { style: "thin", color: { argb: XL.bdr } }, right: { style: "thin", color: { argb: XL.bdr } } }
      if (col.fmt && val != null && !Number.isNaN(Number(val))) {
        cell.value = Number(val); cell.numFmt = col.fmt
      } else {
  ...
  if (totals) {
    sheet.getRow(row).height = 20
    for (let j = 0; j < cols.length; j++) {
      const col = cols[j]; const val = totals[col.key]
      const cell = sheet.getCell(`${colLetter(j + 1)}${row}`)
      cell.fill = xFill(XL.totBg)
      cell.font = xFont({ bold: true, size: 10, color: XL.totTx })
      cell.alignment = xAlign(col.align ?? "left", col.align !== "right" ? 1 : 0)
      cell.border = { top: { style: "medium", color: { argb: XL.blue } }, bottom: { style: "thin", color: { argb: XL.bdr } }, right: { style: "thin", color: { argb: XL.bdr } } }
      if (val == null) { cell.value = "" }
      else if (col.fmt && !Number.isNaN(Number(val))) { cell.value = Number(val); cell.numFmt = col.fmt }
...
function xlKpis(sheet: any, startRow: number, numCols: number, kpis: { totalPrestado: string; totalCobrado: string; totalPendiente: string; activos: number; mora: number; clientes: number }): number {
  const L = colLetter(numCols)
  let row = startRow + 1

  // RESUMEN EJECUTIVO title
  sheet.getRow(row).height = 20
  xMerge(sheet, `A${row}:${L}${row}`, { value: "RESUMEN EJECUTIVO", fill: XL.hdrBg, font: xFont({ bold: true, size: 11, color: XL.hdrTxt }), align: xAlign("left", 2) })
  row++

  // Spacer
  sheet.getRow(row).height = 6; xMerge(sheet, `A${row}:${L}${row}`, { value: "", fill: XL.white }); row++

  // KPI data: 3 per row, 2 rows total → 6 cols used as A:B / C:D / E:F
  const kpiRows = [
    [
      { label: "Total Prestado",    value: fmtRD(kpis.totalPrestado),   color: XL.blue  },
      { label: "Total Cobrado",     value: fmtRD(kpis.totalCobrado),    color: XL.green },
      { label: "Balance Pendiente", value: fmtRD(kpis.totalPendiente),  color: XL.amber },
    ],
    [
      { label: "Préstamos Activos",  value: String(kpis.activos), color: XL.blue },
  ...
  const kpiCols = [[1, 2], [3, 4], [5, 6]] // each KPI spans 2 columns
  ...
  for (const items of kpiRows) {
    ...
    for (let i = 0; i < items.length; i++) {
      const [c1, c2] = kpiCols[i]; const item = items[i]
      ...
      xMerge(sheet, r1, {
        ...
        border: { top: { style: "thin", color: { argb: XL.kpiBdr } }, right: { style: "thin", color: { argb: XL.kpiBdr } }, left: { style: "medium", color: { argb: item.color } } },
      })
      xMerge(sheet, r2, {
        value: item.value,
        fill: XL.kpiBg,
        ...
        border: { bottom: { style: "thin", color: { argb: XL.kpiBdr } }, right: { style: "thin", color: { argb: XL.kpiBdr } }, left: { style: "medium", color: { argb: item.color } } },
      })
    }
    row += 2

    // Gap between KPI rows
    sheet.getRow(row).height = 8; xMerge(sheet, `A${row}:${L}${row}`, { value: "", fill: XL.white }); row++
  }
  return row
}
...
type ClienteSaldo = { cliente: string; telefono: string; empresa: string; saldo: string }

async function generateExcel(params: {
  activos: FlatRow[]; mora: FlatRow[]; clientesSaldo: ClienteSaldo[]
  kpis: { totalPrestado: string; totalCobrado: string; totalPendiente: string; activos: number; mora: number; clientes: number }
  userName: string; now: Date; filters: { fechaDesde?: string; fechaHasta?: string }
}): Promise<Buffer> {
  const { Workbook } = await import("exceljs")
  const wb = new Workbook()
  wb.creator = "Préstamos Elicar"
  wb.created = params.now; wb.modified = params.now

  const nowStr = params.now.toLocaleString("es-DO", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })
  const dateStr = params.now.toLocaleDateString("es-DO", { day: "2-digit", month: "long", year: "numeric" })

  const sumActivoMonto = params.activos.reduce((s, r) => s.plus(r.monto), new Decimal(0))
  const sumActivoPend  = params.activos.reduce((s, r) => s.plus(r.capital_pendiente), new Decimal(0))
  const sumMoraMonto   = params.mora.reduce((s, r) => s.plus(r.monto), new Decimal(0))
  const sumMoraPend    = params.mora.reduce((s, r) => s.plus(r.capital_pendiente), new Decimal(0))
  const avgDias = params.mora.length > 0
    ? Math.round(params.mora.reduce((s, r) => s + diasAtraso(r.fecha_proximo_vencimiento), 0) / params.mora.length) : 0
  const totalSaldoCli  = params.clientesSaldo.reduce((s, c) => s.plus(c.saldo), new Decimal(0))

  // ── Hoja 1: Resumen Ejecutivo ────────────────────────────────────────────
  const shRes = wb.addWorksheet("Resumen Ejecutivo", { views: [{ state: "frozen", ySplit: 5 }] })
  for (let i = 1; i <= 6; i++) shRes.getColumn(i).width = 20
  let y = xlHeader(shRes, 6, "REPORTE GENERAL DEL SISTEMA", params.userName, nowStr, params.filters)
  y = xlKpis(shRes, y, 6, params.kpis)
  shRes.getRow(y).height = 16
  xMerge(shRes, `A${y}:F${y}`, { value: "Ver hojas: Préstamos Activos • Préstamos Mora • Clientes para el detalle completo.", fill: XL.infoBg, font: xFont({ size: 9, color: XL.infoTx }), align: xAlign("left", 1) })
  y++
  xlFooter(shRes, y, 6, dateStr)

  // ── Hoja 2: Préstamos Activos ─────────────────────────────────────────────
  const shAct = wb.addWorksheet("Préstamos Activos", { views: [{ state: "frozen", ySplit: 7 }] })
  ;[5, 28, 18, 18, 18, 8, 14].forEach((w, i) => { shAct.getColumn(i + 1).width = w })
  y = xlHeader(shAct, 7, "PRÉSTAMOS ACTIVOS", params.userName, nowStr, params.filters)
  y++
  const colsAct: XlCol[] = [
    ...
    { header: "Monto Orig.", key: "monto",          width: 18, align: "right",  fmt: '"RD$"#,##0.00' },
    { header: "Cap. Pend.",  key: "cap",            width: 18, align: "right",  fmt: '"RD$"#,##0.00' },
  ...
  const shMora = wb.addWorksheet("Préstamos Mora", { views: [{ state: "frozen", ySplit: 7 }], properties: { tabColor: { argb: "FFDC2626" } } })
  ;[5, 28, 18, 18, 18, 12, 14].forEach((w, i) => { shMora.getColumn(i + 1).width = w })
  y = xlHeader(shMora, 7, "PRÉSTAMOS VENCIDOS / MORA", params.userName, nowStr, params.filters)
  y++
  ...
  const shCli = wb.addWorksheet("Clientes", { views: [{ state: "frozen", ySplit: 7 }] })
  ;[5, 30, 18, 25, 20].forEach((w, i) => { shCli.getColumn(i + 1).width = w })
  y = xlHeader(shCli, 5, "CLIENTES CON SALDO PENDIENTE", params.userName, nowStr, params.filters)
  y++
  const colsCli: XlCol[] = [
    { header: "#",               key: "num",      width: 5,  align: "center" },
...
export async function GET(request: Request) {
  ...
  let rows = prestamos ?? []
  if (f.empresaId)        rows = rows.filter((p) => (p.clientes as { empresa_id?: number })?.empresa_id === f.empresaId)
  if (f.representanteId)  rows = rows.filter((p) => (p.clientes as { representante_id?: number })?.representante_id === f.representanteId)

  // Flatten
  const flat: FlatRow[] = rows.map((p) => {
    const c = p.clientes as { nombre?: string; apellido?: string; cedula?: string; telefono?: string; empresas?: { nombre?: string }; representantes?: { nombre?: string; apellido?: string } }
    return {
      id: p.id as number,
  ...
  for (const r of flat) {
    ...
    if (r.estado !== "SALDADO") { totalPendiente = totalPendiente.plus(r.capital_pendiente); clientesSet.add(r.cedula !== "—" ? r.cedula : String(r.id)) }
  }
  ...
  for (const r of flat) {
    ...
    if (ex) { ex.saldoD = ex.saldoD.plus(r.capital_pendiente) }
    else     { cliMap.set(key, { cliente: r.cliente, telefono: r.telefono, empresa: r.empresa, saldo: r.capital_pendiente, saldoD: new Decimal(r.capital_pendiente) }) }
  }
  const clientesSaldo: ClienteSaldo[] = [...cliMap.values()].sort((a, b) => b.saldoD.cmp(a.saldoD)).map((c) => ({ cliente: c.cliente, telefono: c.telefono, empresa: c.empresa, saldo: c.saldoD.toFixed(2) }))
  ...
  // ── Excel ────────────────────────────────────────────────────────────────
  if (formato === "excel") {
    const excelBuf = await generateExcel({ activos, mora, clientesSaldo, kpis, userName, now, filters })
  ...
  const addPage = (): number => { doc.addPage(); return drawMiniHeader(doc) }
  ...
  doc.setProperties({ title: "Reporte General — Préstamos Elicar", subject: "Microfinanzas y Soluciones Crediticias", author: "Préstamos Elicar", creator: "Préstamos Elicar" })
