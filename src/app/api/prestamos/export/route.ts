...
export async function GET(request: Request) {
  ...
  if (formato !== "csv" && formato !== "xlsx") {
    return NextResponse.json(
      { error: "format debe ser csv o xlsx" },
      { status: 400 },
    );
  }
  ...
  const conInteresPendiente =
    searchParams.get("conInteresPendiente") === "true";

  const pre = await resolvePrestamosListPreFilters(
    supabase,
    qText,
    conInteresPendiente,
  );
  ...
  const flat = rows.map((r) => {
    ...
    return {
      ...
      capital_debitar_proximo: formatRDPlain(
        rr.capital_debitar_proximo as string,
      ),
      ...
      capitalizacion_auto_en_historial: rr.tiene_capitalizacion_auto
        ? "Sí"
        : "No",
      capitalizacion_manual_en_historial: rr.tiene_capitalizacion_manual
        ? "Sí"
        : "No",
    };
  });
