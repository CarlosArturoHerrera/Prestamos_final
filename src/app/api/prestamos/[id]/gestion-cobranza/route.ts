...
export async function POST(request: Request, ctx: Ctx) {
  ...
  const parsed = gestionCobranzaCreateSchema
    .omit({ prestamoId: true })
    .safeParse(body);
  if (!parsed.success) {
    return badRequest(parsed.error.issues[0]?.message ?? "Validación fallida");
  }

  const { notas, promesaMonto, promesaFecha, proximaFechaContacto, resultado } =
    parsed.data;
  ...
  if (!ensuredProfile.ok) {
    console.error(
      "[gestion-cobranza][prestamo] No se pudo asegurar profile antes del insert",
      {
