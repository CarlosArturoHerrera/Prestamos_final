...
export async function POST(request: Request, ctx: Ctx) {
  ...
  const interesCalculadoStr = interesPeriodo(
    capitalAntes,
  ...
  const interesRecibidoStr =
    parsed.data.interesRecibido !== undefined
      ? String(parsed.data.interesRecibido)
      : "0";
  ...
  const diferenciaPendiente = Decimal.max(
    new Decimal(0),
    interesCalculado.minus(interesAplicado),
  );

  const capitalDebitado = new Decimal(
    parsed.data.montoCapitalDebitado !== undefined
      ? String(parsed.data.montoCapitalDebitado)
      : "0",
  );
  ...
  if (capitalDebitado.gt(new Decimal(capitalAntes))) {
    return badRequest(
      "El capital a debitar no puede superar el capital pendiente",
    );
  }

  const totalPagado = interesAplicado.plus(capitalDebitado);

  const saldoCapitalRestante = subDecimal(
    capitalAntes,
    capitalDebitado.toFixed(2),
  );
  const saldoFinal = capitalPendienteFinal(saldoCapitalRestante);

  const nuevoEstado = new Decimal(saldoFinal).lte(0)
    ? "SALDADO"
    : prestamo.estado === "MORA"
      ? "ACTIVO"
      : prestamo.estado;

  const nuevaFechaProximo = new Decimal(saldoFinal).lte(0)
    ? prestamo.fecha_proximo_vencimiento
    : siguienteVencimientoDesde(
        String(prestamo.fecha_proximo_vencimiento),
        prestamo.tipo_plazo,
      );
