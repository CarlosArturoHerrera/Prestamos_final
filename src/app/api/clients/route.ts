...
export async function GET(request: Request) {
  ...
  if (search) {
    const like = `%${search}%`;
    const segFiltered = await supabase
      .from("segments")
      .select("id")
      .ilike("name", like);
  ...
  let clientsQuery = supabase
    .from("clients")
    .select("id, name, phone, email, location, notes, segment_id, created_at")
  ...
  const [
    ...
    supabase
      .from("payments")
      .select(
        "loan_id, amount_paid, amount_due, status, paid_at, due_date, created_at",
      ),
  ]);

  const segmentMap = new Map((segments ?? []).map((s) => [s.id, s.name]));
  const loansByClient = (loans ?? []).reduce<Record<string, typeof loans>>(
    (acc, loan) => {
  ...
  const paymentsByLoan = (payments ?? []).reduce<
    Record<string, typeof payments>
  >((acc, payment) => {
  ...
  const result: Client[] = (clients ?? []).map((client) => {
    ...
    const statuses = clientLoans
      .flatMap((loan) => paymentsByLoan[loan.id] ?? [])
      .map((p) => p.status);
    ...
    const riskLevel: Client["riskLevel"] = hasMora
      ? "Alerta"
      : hasVencido
        ? "Vigilancia"
        : "Estable";
