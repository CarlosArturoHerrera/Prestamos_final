...
export async function GET() {
  try {
    ...
    const { data: payments } = await supabase
      .from("payments")
      .select("loan_id, status, amount_due, amount_paid");

    // Mapear datos
    const segmentMap = new Map(
      (segments ?? []).map((s: any) => [s.id, s.name]),
    );
    const loansByClient = (loans ?? []).reduce<Record<string, any[]>>(
      (acc, loan) => {
    ...
    const paymentsByLoan = (payments ?? []).reduce<Record<string, any[]>>(
      (acc, payment) => {
    ...
    const result: ClientRow[] = (clients ?? []).map((client: any) => {
      ...
      const loanPayments = clientLoans.flatMap(
        (loan) => paymentsByLoan[loan.id] || [],
      );
      const hasDefaulted = loanPayments.some(
