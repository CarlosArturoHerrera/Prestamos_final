...
import type {
  PortfolioMetric,
  PortfolioPoint,
} from "@/components/dashboard/portfolio-chart-card";
import type {
  ClientRow,
  LoanRow,
  NotificationItem,
} from "@/components/dashboard/segments-tabs";
...
export async function fetchDashboardData(): Promise<DashboardData> {
  ...
  const [
      ...
      supabase
        ...
        .then(async (result) => {
          if (result.data) {
            const total = result.data.reduce(
              (sum, payment) =>
                sum + ((payment.amount_due || 0) - (payment.amount_paid || 0)),
              0,
            );
            return { data: [{ total }], error: result.error };
          }
    ...
    assertNoError<ClientCountRow>(
      supabase
        ...
        .then(async (result) => {
          if (result.data) {
            const uniqueClients = new Set(result.data.map((l) => l.client_id))
              .size;
              ...
              .then((loansResult) => {
                if (loansResult.data) {
                  const uniqueClients = new Set(
                    loansResult.data.map((l) => l.client_id),
                  ).size;
                  return {
    ...
    assertNoError<PaymentDbRow>(
      supabase
        .from("payments")
        .select(
          "loan_id, due_date, amount_due, amount_paid, paid_at, status, created_at",
        )
        .order("created_at", { ascending: false })
        .then(),
      "payments",
  ...
  // Build helper maps
  const loansByClient = loans.reduce<Record<string, LoanDbRow[]>>(
    (acc, loan) => {
  ...
  const paymentsByLoan = payments.reduce<Record<string, PaymentDbRow[]>>(
    (acc, payment) => {
  ...
  const clientRows: ClientRow[] = clients.map((client) => {
    ...
    const balance = clientLoans.reduce(
      (sum, loan) => sum + (loan.principal ?? 0),
      0,
    );

    const hasLatePayments = clientLoans.some((loan) =>
      (paymentsByLoan[loan.id] || []).some(
        (p) => p.status === "mora" || p.status === "vencido",
      ),
    );
  ...
  const notificationItems: NotificationItem[] = payments
    ...
    .map((p) => {
      ...
      const clientName =
        clients.find((c) => c.id === loan?.client_id)?.name ?? "";
      const title =
        p.status === "mora" || p.status === "vencido"
          ? "Pago vencido"
          : "Pago pendiente";
